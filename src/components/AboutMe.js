            ...
            if (!hero || (Date.now() - hero.time) > periodMonth) {
                fetch(characters[key].url)
                    ...
                    .then(data => {
                        ...
                        this.setState({existHero: true, hero: info});
                        hero = {
                            info,
                            time: Date.now()
                        };
                ...
                this.setState({existHero: true, hero: hero.info});
            }
            this.props.changeHero(key);
        }else {
            this.setState({existHero: false, hero: defaultHero});
            this.props.changeHero(defaultHero);
        }
    }
